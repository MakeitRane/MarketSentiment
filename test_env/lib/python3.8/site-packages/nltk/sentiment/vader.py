...
class SentimentIntensityAnalyzer:
    ...
    def polarity_scores(self, text):
        ...
        words_and_emoticons = self.comma_check(words_and_emoticons)
        words_and_emoticons = self.dollar_check(words_and_emoticons)
        words_and_emoticons = self.k_convert(words_and_emoticons)
        ...
        sentiments = self.coin_comparison(words_and_emoticons,self.coin_dict,sentiments)
        return self.score_valence(sentiments, text)
    
    def comma_check(self, words_and_emoticons):
        i=0
        for item in words_and_emoticons:
            words_and_emoticons[i] = re.sub(",", "", item)
            i+=1
        return words_and_emoticons
            
    def dollar_check(self, words_and_emoticons):
        i=0
        for item in words_and_emoticons:
            if((item[0])=='$' and (item[1:].isdigit())):
                words_and_emoticons[i] = item[1:]
            i+=1
        return words_and_emoticons
    def k_convert(self, words_and_emoticons):
        j=-1
        for item in words_and_emoticons:
            num_item = ""
            i = 0
            ...
            for chara in item:
                if(item[i].isdigit()):
                    num_item = num_item + item[i]
                    i+=1
                    continue
                elif(item[i] == "$"):
                    i+=1
                    continue
                elif(item[i] == 'K' or item[i] == 'k'):
                    num_item2 = int(num_item) *1000
                    item = num_item2
                    break
                else:
                    break
            words_and_emoticons[j] = str(item)
        return(words_and_emoticons)  
    coin_dict = {"$BTC": 30000.00, "$ETH": 2000.00, "$BNB": 250.00, "$XRP": 0.75, "$ADA": 0.33, "$SOL": 27.32, "$MATIC": 0.80, "$DOT": 5.42, "$AVAX": 14.98,
            "$LINK": 6.92, "$ATOM": 9.69, "$UNI": 5.79, "$ARB": 1.26, "$APT": 7.43, "$NEAR": 1.50, "$AAVE": 78.91, "$OP": 1.45, "ALGO": 0.11}
    def coin_comparison(self, words_and_emoticons, coin_dict, sentiment):
        i = 0
        for word in words_and_emoticons:
            if word in coin_dict:
                for j in range(i,len(words_and_emoticons)-1):
                    if words_and_emoticons[j].isdigit():
                        sent1 = self.coin_math(int(words_and_emoticons[j]), word)
                        sentiment[j] = sent1
                        if words_and_emoticons[j+1].isdigit():
                            sent2 = self.coin_math(int(words_and_emoticons[j+1]), word)
                            sentiment[j+1] = sent2
                    else:
                        continue
            i = i+1
    ...
    def coin_math(self, intake, word):
        number = self.coin_dict[word]
        frac = number/intake
        senti = ((-1.8824*(frac)+1.8824)/(frac+0.4559))
        return senti
    ...
    def sentiment_valence(self, valence, sentitext, item, i, sentiments):
        ...
        if item_lowercase in self.lexicon:
            ...
            #valence = self._but_check(words_and_emoticons, sentiments)
    ...
    def print_sent(self, sentiments):
        for item in sentiments:
            print(item)
    def print_valence(self, valence):
        print(valence)
